...
public class ProjectSearchTest {
    ...
    // Indexing is causing a problem in search tests - use this to clean
    // indexes before test execution
    public void indexWorkaround() {
        new LoginWorkFlow().signIn("admin", "admin").goToAdministration()
                .goToManageSeachPage().clickSelectAll()
                .performSelectedActions().logout();
    }
}
